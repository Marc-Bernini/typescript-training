...
there!`;
    ...
    result = 'This is `escaped` backtics';
    expect(result).toBe('This is `escaped` backtics')
  })
  ...
  it('should call the tagging function', () => {
    ...
    function tagIt(literalString, ...interpolatedParts) {
      // implement this function to make the test pass
        let result = `${literalString[0]}dear ${interpolatedParts[0]}${literalString[1]}really ${interpolatedParts[1]}${literalString[2]}`
        return result ;
    };
  });
  ...
  it('can be curried', () => {
    ...
    let journey = country1 => country2 => country3 => `${country1}, then ${country2} and finally ${country3}!`;
